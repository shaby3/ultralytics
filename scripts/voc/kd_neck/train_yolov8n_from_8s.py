# python scripts/voc/kd_neck/train_yolov8n_from_8s.py
"""KD training — yolov8n student, yolov8s(VOC 학습본) teacher, neck 출력(P3/P4/P5)에서 증류.

Phase 1(증류 위치 비교) 3런 중 하나. teacher·aligner·loss 는 세 런 공통이고
layers 와 weight 만 다르다 — ultralytics/cfg/distill_neck_from_8s_voc.yaml 참조.
weight 20 은 위치별 kd_loss 실측을 head1 기준으로 정규화한 값이다 (README §4).

EMA 유령 hook 리크 수정 후 head1 실측 7.8분/에폭(batch 16, peak 2.9G/6.1G) — README §7.8.
