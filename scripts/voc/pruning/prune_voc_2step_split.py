...
if __name__ == '__main__':
    ...
    prunetrain(
        ...
        train_epochs=50,            # 50×2 = 100 epoch 합계 (split 전략)
        prune_epochs=50,
        imgsz=640,
        batch=64,
        device=[0],
        name='yolo26n_voc_2step_split_50ep',
        prune_ratio=0.5,            # Pruning Ratio (50%)
        prune_iterative_steps=2,    # 2-step iterative pruning
        patience=0,                 # 0 = early stopping 비활성화 (할당 epoch 전부 사용)
        workers=2,
        project='voc/pruning/2step_split',
    )
