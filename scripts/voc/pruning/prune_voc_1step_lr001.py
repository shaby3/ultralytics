...
def iterative_prune(model: YOLO, data, finetune_epochs, target_prune_ratio,
    ...
    for step in range(iterative_steps):
        ...
        _train_pruned(model, name=f'{name}_pruned_step{step}', data=data,
                      ...
                      project=project, workers=workers, lr0=0.001, lrf=0.1, warmup_epochs=1.0, warmup_bias_lr = 0.01)
...
if __name__ == '__main__':
    ...
    prunetrain(
        ...
        batch=64,                  # Batch size (VOC: GC10-DET 대비 클래스 2배, 이미지 수 多)
        device=[0],                # GPU devices
        name='yolo26n_voc_1step_lr001_100epochs',  # Save name
        prune_ratio=0.5,           # Pruning Ratio (50%)
        prune_iterative_steps=1,   # Pruning Iterative Steps
        workers=2,
        project='voc/pruning/1step_lr001',
    )
