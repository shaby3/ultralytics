# python scripts/voc/kd_head0/train_yolov8n_from_8s_coco.py
"""KD training — yolov8n student, yolov8s **COCO-pretrained** teacher, Detect head 1번째 conv.
...
같은 위치 스윕의 나머지 둘 — teacher·aligner·loss 가 공통이고 layers 와 weight 만 다르다:
  scripts/voc/kd_neck/train_yolov8n_from_8s_coco.py
  scripts/voc/kd_head1/train_yolov8n_from_8s_coco.py  (완료, mAP50-95 0.6496)
head0 은 head1 과 채널이 같아 aligner 까지 동일하다 — 세 수준 중 가장 깨끗한 비교(깊이만 다르다).
weight 10 은 위치별 kd_loss 실측을 head1 기준으로 정규화한 값이다 — README §4 참조.

teacher 만 다른 통제 런: train_yolov8n_from_8s.py (teacher s-VOC) — 아직 미실행.
설정을 하나라도 건드리면 위치 비교가 교락되므로, 세 런을 함께 바꿀 것.

head1 실측 7.8분/에폭(batch 16, peak 2.9G/6.1G) — head0 은 동급이다. 100에폭 약 13시간.

결과: runs/detect/voc/kd_head0/yolov8n_from_8s_coco/{train,val}/
"""
...
PROJECT = "voc/kd_head0/yolov8n_from_8s_coco"
...
if __name__ == "__main__":
    ...
    trainer = Distiller(
        overrides={
            ...
            "device": 0,
            "amp": True,
            "distill_cfg": "ultralytics/cfg/distill_head0_from_8s_coco.yaml",
        }
    )
    trainer.train()
