...
kd_loss 는 배치 평균이라 batch 크기와 무관하고, 트레이너가 weight 를 곱하기 전 값을 기록하므로
(distiller.py 의 `kd_val = kd_loss.detach().item()`) 프로브의 weight=1.0 은 측정에 영향이 없다.
