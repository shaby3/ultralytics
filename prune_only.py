...
def iterative_prune(model: YOLO, data, finetune_epochs, target_prune_ratio,
    ...
    init_map = model.val(data=data, imgsz=imgsz, batch=batch, device=device, project=project, workers=workers, name='val_init').box.map
    gc.collect()
    ...
    for step in range(iterative_steps):
        ...
        # save ckpt path before clearing trainer
        last_ckpt = str(model.trainer.best if model.trainer.best.exists() else model.trainer.last)
        model.trainer = None
        gc.collect()
        torch.cuda.empty_cache()

        current_map = model.val(data=data, imgsz=imgsz, batch=batch, device=device, project=project, workers=workers, name=f'val_step{step}').box.map
        gc.collect()
        torch.cuda.empty_cache()
...
if __name__ == '__main__':
    ...
    prunetrain(
        ...
        train_epochs=50,           # Epochs before pruning
        ...
        project='./runs/detect/pruning_resume_50epoch_2step'
    )
    ...
    #            prune_ratio=0.5, prune_iterative_steps=1, workers=0)
