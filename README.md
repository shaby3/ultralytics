...
| 12 | head`.1` × **m**-COCO × PKD | teacher 크기 (기법 2종으로 교차 확인) | 대기 — 프로브 후 |
...
### KD — teacher 크기 축 (head`.1` × QMSE 고정, 100에폭, batch 16)
...
**한계 — 기법 한 종(QMSE)에서만 본 것이다.** "m 이 나쁜가" vs "QMSE×m 조합이 나쁜가"를 가르려면
다른 기법이 필요해서 **m×PKD 런(12번)**을 추가한다. PKD 에서도 m 이 뒤지면 기법 무관한 크기 효과로
굳고, PKD 에서 m 이 이기면 기법 × teacher 크기 교호작용이다.
