...
| 5 | head`.1` × s-COCO × **PKD** | KD 기법 | 완료 0.6533 |
| 6 | head`.1` × s-COCO × **MGD** | KD 기법 | 완료 0.6501 (≈MSE) |
| 7 | head`.1` × s-COCO × **FGD** | KD 기법 | **중단** — ep2 에서 폐기 ([§4](#fgd-는-시도-후-중단했다)) |
| 8 | head`.1` × s-COCO × **AMSE** | KD 기법 (FGD ablation) | 완료 0.6429 (**< MSE**, 100/100 열세) |
| 9 | head`.1` × s-COCO × **QMSE** | KD 기법 (가중 기준: 크기→품질) | 완료 **0.6564** ← 기법 선두 |
| 10 | head`.1` × s-COCO × **PQMSE** | KD 기법 (마스크 구성: 성분→결합) | **중단** — ep17 에서 폐기 ([§4](#pqmse--결합-품질-마스크-pgd-식-score_gt클래스05--iou2)) |
| 11 | head`.1` × **m**-COCO × QMSE | teacher 크기 (기법은 승자 고정) | 완료 0.6517 (**< s** −0.47pt) |
| 12 | head`.1` × **m**-COCO × PKD | teacher 크기 (기법 2종으로 교차 확인) | 완료 0.6541 (**≈ s**, +0.08pt) |
| 13 | **neck** × s-VOC × MSE | 위치 × teacher 출처 2×2 완성 | 완료 0.6372 (**neck > head`.1`** — 순위 역전) |
...
**Q3(teacher 크기)은 기법 축이 닫힌 뒤 승자 기법으로 돈다** — 기법을 QMSE 로 고정하고
teacher 만 s→m 으로 바꾼 **s-COCO×QMSE(0.6564)** 와의 한 축 비교다 (11번 런).
teacher 크기 추이는 3점으로 읽는다: **teacher 없음(baseline n 0.6284) → s(0.6564) → m.**
teacher 가 클수록 나빠지는 구간이 보이면 capacity gap 을 짚는 근거가 된다.

**기법 2종으로 교차 확인한다 (12번 런).** m×QMSE 가 ep24~48 구간에서 s×QMSE 에 −0.3~−0.9pt 로
안정적으로 뒤지는 궤적이라, 이대로면 "m 이 나쁘다"와 "QMSE×m 조합이 나쁘다"가 구분되지 않는다.
**PKD 로도 m 이 s 에 뒤지면 기법과 무관한 teacher 크기 효과(capacity gap)로 굳고**, PKD 에서는
m 이 이기면 기법 × teacher 크기 교호작용이 있다는 뜻이다. 2×2 가 채워진다:
s×QMSE 0.6564 / s×PKD 0.6533 / m×QMSE / m×PKD.
...
**위치 순위는 teacher 에 의존한다 — greedy 탐색의 한계가 실제로 드러났다.** 13번 런(neck × s-VOC)이
2×2 를 채우자 **순위가 뒤집혔다**: COCO teacher 에서는 head`.1` 이 neck 보다 +0.88pt 앞서지만,
VOC teacher 에서는 neck 이 오히려 앞선다(+0.29pt, 96/100 에폭). 교호작용 **+1.17pt**.
같은 사실을 뒤집어 읽으면 **COCO 사전학습의 이득이 head 분기에 집중**되어 있다는 뜻이다
(head`.1` 에서 +1.53pt vs neck 에서 +0.36pt). 상세는 [§6](#위치--teacher-출처-2x2--위치-순위는-teacher-에-의존한다).
**기법 순위는 여전히 head`.1` 에서만 확인한 것이다.**
...
| **AMSE** (T=1) | loss 만 (FGD ablation, 아래) | `..._coco_amse.yaml` | **0.1** | 완료 0.6429 (< MSE) |
| **QMSE** | loss 만 (가중 기준: 품질, 아래) | `..._coco_qmse.yaml` | **0.57** | 완료 **0.6564** ← 선두 |
| **PQMSE** | loss 만 (마스크 구성: 결합, 아래) | `..._coco_pqmse.yaml` | **0.5** | **중단** (ep17, 아래) |
...
#### AMSE — FGD 의 attention 가중만 분리한 변형

FGD 부검이 남긴 질문을 실험으로 바꾼 것이다. FGD 의 focal 항은 **① GT 전경/배경 마스크
② teacher attention 가중 ③ attention 강제 L1(γ항)**의 묶음이었는데, head 붕괴의 범인이
①③인지 ② 자체인지 이 묶음으로는 가릴 수 없다. **AMSE 는 ②만 남긴 MSE 다:**

```
w_s = H·W·softmax(|t|의 채널평균 / T)    # 공간 가중, 평균 1
w_c = C·softmax(|t|의 공간평균 / T)      # 채널 가중, 평균 1
loss = mean(w_s · w_c · (s − t)²)        # T = 1.0 (FGD 기본값 0.5 는 head 에서 퇴화 — 아래)
```

- **논문의 단독 기법이 아니라 자체 변형(ablation)이다** — "SOTA 적용"이 아니라
  "실패 원인을 가설로 세워 구성요소를 분리 검증"으로 서술해야 한다.
- **가중치는 teacher 로만** 계산한다. student attention 을 넣으면 안 맞는 곳의 자기 attention 을
  줄이는 우회가 생긴다. teacher 는 detach 라 가중치가 상수다. γ항은 넣지 않는다 —
  넣는 순간 "가중만 분리한" 실험이 아니게 된다.
- 두 가중치 모두 축 평균이 **정확히 1**이라 mean reduction 관례 안이고, attention 이 균등하면
  일반 MSE 로 환원된다 (FGD 가 sum 관례 예외를 요구했던 것과 대비된다).
  파라미터·GT 불필요라 PKD 와 같은 무상태 loss 슬롯에 들어간다.
- **가중 평균 1 ≠ 실효 배율 1.** 오차가 attention 과 무관한 합성 노이즈에서는 MSE 대비
  0.95~1.07배지만(`E[w·e²] = E[e²]`), 1에폭 프로브 실측은 **15배(78.1 vs 5.26)**였다.
  학습 초기 student 의 오차가 teacher attention 이 높은 위치에 몰려 있다는 뜻이다 —
  attention 가중이 수식보다 훨씬 날카로운 재배분이라는 실측 증거이자, 같은 가중을 쓰는
  FGD 의 fea_loss 가 초반 학습을 압도한 메커니즘의 단서다. 이 때문에 w=1.0 프로브는
  KD 비중 93% 로 task 가 오염됐고(5.92 vs 4.55) FGD 처럼 두 번 재서 weight 를 정했다.
- **head feature 에서 T=0.5 의 channel attention 은 거의 one-hot 으로 퇴화한다** (실제 VOC 이미지 실측).
  spatial `w_s` 는 최대 12~40, 상위 1% 위치가 질량의 7~18%를 갖는 "뾰족하지만 상식적인" 분포인데,
  channel `w_c` 는 최댓값이 C 에 근접한다 — box P4 에서 62.5/64, cls P5 에서 **127.3/128**.
  head 분기는 채널별 평균 활성의 격차가 크고 T=0.5 가 그 격차를 exp 에서 증폭한 결과다.
  FGD 가 검증된 넥 FPN(256채널, BN 으로 고른 분포)에서는 같은 수식이 훨씬 부드럽다 —
  off-label 문제의 또 다른 단면. **그래서 T=1.0 으로 완화해서 돌린다** (사용자 결정) —
  FGD 정의(0.5)에서 한 발 더 벗어난 변형이 되지만, 퇴화한 수식의 확인사살보다
  "attention 가중이 head 에서 유효한가"에 답하는 쪽을 택했다.
- **weight 는 0.1 로, T=0.5 프로브의 공식 산출값(0.0687)보다 1.46배 높게 사용자 결정.**
  T=1 에서는 가중이 부드러워져 kd_loss 원값이 15배(T=0.5 실측)보다 내려가므로 재프로브 없이는
  실효 KD 비중을 모른다 — ep1 의 가중 kd_loss 로그로 실측 확인하고, MSE 대비 결과 해석에는
  "가중 방식 + KD 강도 + T" 가 함께 움직였다는 단서가 붙는다.
- MSE 런과 **정확히 한 축**(균등 가중 → attention 가중)만 다르다 — MGD(3축)·FGD(off-label+3축)보다
  깨끗한 비교다.
- 판정: AMSE 가 MSE 를 넘으면 FGD 실패의 범인은 ①③으로 좁혀지고, AMSE 도 무너지면
  attention 가중 자체가 head 와 안 맞는 것이다. **어느 쪽이든 FGD 중단 기록의 해석이 단단해진다.**
  FGD 경험에 따라 ep1 이 역대 초반 밴드(0.39~0.45) 밖이면 조기 중단한다.
- **→ 결과: 0.6429, MSE 에 −0.66pt·100/100 에폭 열세로 후자였다.** 초반 붕괴 없이(ep1 밴드 안)
  완주했지만 순위는 끝까지 뒤집히지 않았다 — attention 가중은 head 분기에서 이득을 주지 못한다.
  PKD(크기 제거, +0.37pt)와의 대비 포함 상세 해석은 [§6](#kd--기법-스윕-head1--s-coco-고정-100에폭-batch-16).

#### QMSE — 가중 기준을 activation 크기에서 예측 품질로

AMSE 부검의 다음 질문이다: 공간 가중 자체가 무효인가, 아니면 **기준이 틀렸던** 것인가.
QMSE 는 branch 별 품질 마스크로 가중한 MSE 다 — MSE(균등)·AMSE(크기)와 "가중 기준" 한 축만 갈린다.

- **box 분기(cv2) ← IoU 마스크**: 위치별 teacher 디코딩 박스 vs 이미지 내 전체 GT 의 max IoU.
  할당이 아니라 품질 측정이라 max 로 충분하다. 배경 위치도 예측 박스가 GT 와 겹치는 만큼 참여한다.
- **cls 분기(cv3) ← score 마스크**: 위치별 teacher cls sigmoid 의 클래스 max (teacher 가 COCO 라
  80클래스 기준 확신도). branch 매핑은 config layers 의 cv2/cv3 에서 자동 판별된다.
- **PGD([arXiv 2203.05469](https://arxiv.org/abs/2203.05469), ECCV 2022)의 cls/reg 마스크 분리를
  차용한 자체 변형이다** — PGD 는 `s^(1−α)·IoU^α` 로 GT 별 top-k 를 선발해 2D 가우시안을 적합하고
  FGD 식 fg/bg+attention 에 얹지만, 여기서는 성분 마스크를 전 픽셀 soft 가중으로 직접 쓴다
  (top-k·가우시안·fg/bg·attention 없음). "SOTA 이식"이 아니라 가설 검증용 최소 형태다.
- 마스크는 teacher 예측+GT 로만 계산되는 상수(no_grad)고 지점·이미지별 **평균 1 정규화** —
  mean 관례 안이고 균등 마스크면 MSE 로 환원된다. FGD(박스당 총합 1)·PGD(비-0 셀 수로 나눔)가
  sum 세계에서 하던 정규화의 mean 세계 번역이며, FGD 의 소형 객체 강조는 이 번역에 없다.
  채널 가중도 없다 (AMSE 에서 one-hot 퇴화가 확인된 축). 실측: 실제 이미지에서 IoU 마스크
  질량의 96.7%가 GT 영역 안에 몰린다 (균등 기대치 67.6%).
- 해석 단서: 마스크에 GT·teacher 예측이 들어가므로 feature 만 보는 수준들과 입력 정보량이 다르다.
- **집중도 실측 (VOC 16장, 정규화 후):** 두 마스크의 성격이 극적으로 다르다.
  IoU 마스크는 온건하다 — 유효 참여율(엔트로피 기준) 44~56%, 상위 1% 질량 4~12%.
  **score 마스크는 매우 집중적이다** — 참여율 4~8%, 상위 1%가 질량의 31~74%, 셀의 72~82%가
  w<0.1 로 사실상 배제 (teacher 배경 score 10⁻²~10⁻³ vs 객체 0.5~0.9 의 수백 배 비율 때문).
  즉 cls 분기 증류는 "teacher 가 객체라고 확신하는 소수 위치"에서만 일어나고 배경 억제
  지식은 전달되지 않는다. **의도된 형태로 판단해 그대로 간다** — "품질 집중"이 가설 자체고
  PGD 도 top-k 극소수만 증류한다. AMSE 의 패인은 집중이 아니라 집중한 기준(크기)이었다는 게
  현재 그림이다. 지면 이 표가 원인 규명의 1순위 후보다 (완화 변형 √score 등은 그때).
- 판정: 이기면 "품질 가중 유효" — 크기 가중(AMSE)과의 대비가 완성된다. 지면 "head 증류에
  공간 가중 자체가 무효 — 순수 채널 패턴(PKD)만 남는다". 조기 판정 기준은 AMSE 와 동일
  (ep1 밴드 0.39~0.45).
- **→ 결과: 0.6564 — 기법 축 신기록 (MSE +0.69pt·91/100 우세, PKD +0.31pt·ep60 이후 34/41).**
  "품질 가중 유효"가 확정됐고, 가중 기준 축이 단조로 닫혔다: 품질 > 균등 > 크기.
  상세는 [§6](#kd--기법-스윕-head1--s-coco-고정-100에폭-batch-16).

#### PQMSE — 결합 품질 마스크 (PGD 식, score_GT클래스^0.5 × IoU^2)

QMSE 승리 후의 세부 형태 질문이다: **성분 마스크(분기별)와 결합 마스크(쌍 단위 곱) 중 어느 쪽이
나은가.** QMSE 와 마스크 구성 두 축이 갈린다 — ① 분기별 → 6지점 공통 결합, ② 클래스 max →
**GT 클래스의 score** (PGD·TAL 과 같은 쌍 정합: `t(위치,GT) = s_GT클래스^0.5 × IoU^2`, GT축 max).

- **PGD 의 결합 품질 점수 형태를 차용한 변형** — 지수 0.5/2 는 자체 선택(PGD 는 `s^(1−α)·IoU^α`
  보간), top-k·가우시안·fg/bg·attention 은 여전히 없다. IoU 는 일반 IoU (PGD 의 BboxOverlaps2D
  기본값 — TAL 의 CIoU 와 다름).
- **teacher 가 COCO(nc=80)라 VOC 라벨로 score 채널을 직접 인덱싱하면 엉뚱한 클래스를 잡는다**
  (person = VOC 14 vs COCO 0). `VOC_TO_COCO` 매핑 20개로 변환하고, 이름 문자열 자동 대조로
  검증했다. 합성 테스트에서 오답 클래스 위치의 t 가 이론값(√(0.01/0.81)=0.111)대로 죽는 것 확인.
- 집중도 실측: 유효 참여율 7~22% — QMSE 의 score 마스크(4~8%)보다 완화, IoU 마스크(44~56%)보다
  집중. score^0.5 완화와 IoU^2 강조가 상쇄된 중간 지점이다.
- 판정: QMSE 를 넘으면 "쌍 결합(클래스 정합 포함)이 성분 분리보다 낫다", 지면 "분기별 성분
  마스크가 낫다" — 품질 가중의 세부 형태 축이 닫힌다. 조기 판정 기준 동일 (ep1 밴드 0.39~0.45).
- **→ ep17 에서 중단·폐기 (사용자 결정).** ep1 은 밴드 안(0.4188)이었지만 ep4 이후 전 구간에서
  MSE 보다도 ~1pt 낮게 깔렸다 (ep17: 0.5488 vs mse 0.5595 vs qmse 0.5578) — AMSE 와 MSE 사이 궤적.
  세 축(쌍 결합·GT 클래스 한정·지수)이 함께 바뀌어 범인 분리는 안 되지만, **GT 클래스 한정이
  유력 용의자다** — Q1(COCO 일반 표현의 우위)의 논리대로라면 teacher 의 확신을 GT 클래스로 좁히는
  건 다크 널리지를 버리는 방향이다. QMSE 의 클래스 max("뭐든 확신하는 위치")가 그 지식을 살린다.
  분리 검증(GQMSE)은 하지 않기로 했다 — 기법 축을 QMSE 로 닫고 teacher 크기 축으로 이동.
...
| amse | 79.679 | 94.4% | 0.0687 | **0.1** | 62.7% | 2.80 GB | 10.9 |
| qmse | 9.5403 | 66.9% | 0.5714 | **0.57** | 53.5% | 2.89 GB | 8.1 |
| pqmse | 10.499 | 68.4% | 0.5328 | **0.5** | 52.0% | 2.89 GB | 6.9 |
| qmse×m | 9.1200 | 65.9% | 0.5990 | **0.6** | 53.6% | 3.27 GB | 10.2 |
| pkd×m | 0.5775 | 11.1% | 9.2312 | **9.0** | 53.0% | 3.28 GB | 10.0 |
| neck×s-VOC | 0.2283 | 4.7% | 23.54 | **24.0** | 54.2% | 2.68 GB | 7.0 |

(아래 두 행은 기법 축이 아니라 **teacher 크기 축** 런이다 — m teacher 의 forward 가 무거워
에폭이 s 대비 +2분쯤 느리고, VRAM 은 3.27GB 로 batch 16 이 그대로 들어간다.)

> **teacher 크기가 kd_loss 스케일을 거의 안 바꾼다.** QMSE 는 9.54→9.12(−4%), PKD 는
> 0.6476→0.5775(−11%)로, weight 도 0.57→0.6 / 8.0→9.0 수준만 움직였다. 기법을 바꿀 때
> weight 가 0.05~8.0 을 오간 것과 대비된다 — **teacher 크기 비교가 "KD 강도" 교락 없이
> 성립한다는 방증**이다. PKD 쪽 −11% 는 `1 − r` 기준이라 **student 가 m 의 채널 패턴을
> s 보다 오히려 더 잘 따라간다**는 뜻이라, m×QMSE 의 −0.47pt 와 대조하면 흥미로운 지점이다.

(amse 는 FGD 처럼 두 번 쟀다 — w=1.0 첫 프로브는 KD 비중 93% 로 task 가 오염되어, 보정 추정치
0.067 로 재프로브한 값이 위 행이다. 상세는 [AMSE 소절](#amse--fgd-의-attention-가중만-분리한-변형).
**채택 w 0.1 은 산출값의 1.46배**로 정규화 규칙의 반올림 폭(~6%) 밖이다 — 초기 KD 비중이
MSE 보다 9pt 높아, MSE 대비 결과에 가중 방식 효과와 KD 강도 효과가 섞인다는 단서를 달고 읽어야 한다.)
...
Phase 3 에서 축이 하나 늘었다. `yolov8n_from_8s_coco_pkd` / `_mgd` / `_amse` / `_qmse` / `_pqmse` 처럼 **teacher 뒤에 기법을 붙인다.**
...
**⚠️ 이 순위는 s-COCO 에서만 성립한다.** 아래 2×2 가 보여주듯 teacher 를 VOC 학습본으로 바꾸면
**순위가 뒤집힌다.** 여기서 우려로만 적어뒀던 "다른 teacher 에서도 같은 순위인가"가 실제 문제였다.

### 위치 × teacher 출처 2×2 — 위치 순위는 teacher 에 의존한다

13번 런(neck × s-VOC)으로 마지막 칸을 채운 결과다. 네 런 모두 100에폭·batch 16·MSE·ConvBNSiLU 고정.

| mAP50-95 | s-VOC | s-COCO | Δ (COCO − VOC) |
|---|:---:|:---:|:---:|
| **neck** | **0.6372** | 0.6408 | +0.36pt |
| **head`.1`** | 0.6342 | **0.6496** | **+1.53pt** |
| Δ (head − neck) | **−0.29pt** | **+0.88pt** | 교호작용 **+1.17pt** |

**순위가 뒤집힌다.** COCO teacher 에서는 head`.1` 이 확실히 앞서지만(+0.88pt, ep20 이후 부호 불변),
VOC teacher 에서는 **neck 이 앞선다** — 격차 +0.29pt 는 노이즈 기준(0.3pt) 바로 아래라
"동률"로 읽는 게 정확하지만, **96/100 에폭에서 neck 이 위**라 방향만큼은 일관적이다.
어느 쪽이든 "head 가 낫다"는 COCO teacher 한정 결론이다.

**같은 표를 세로로 읽으면 더 유용한 해석이 나온다 — COCO 사전학습의 이득이 head 분기에 집중돼 있다.**
teacher 출처를 바꾼 효과가 neck 에서는 +0.36pt(노이즈 수준)인데 head`.1` 에서는 **+1.53pt** 다.
Q1 의 "COCO teacher 가 낫다"는 결론이 사실은 **"COCO teacher 의 head 분기 특징이 낫다"** 였던 셈이다.
해석하면: VOC 로 파인튜닝하면서 head 는 20클래스에 특화되어 student 에게 줄 일반적 표현을 잃지만,
neck 의 공간 표현은 상대적으로 덜 손상된다 — 그래서 VOC teacher 를 쓸 거면 neck 에서 증류하는 게
차라리 낫고, head 에서 증류하려면 COCO 사전학습본이어야 한다.

**실무 함의**: "증류 위치"와 "teacher 출처"는 독립적으로 고를 수 없다. 이 프로젝트가 greedy 탐색
(한 축씩 고정하며 진행)을 택했기 때문에 드러난 한계이고, 2×2 를 채우지 않았으면
"head 가 항상 낫다"로 잘못 일반화할 뻔했다. 기법 × teacher 크기 축에서도 같은 일이 있었다
([아래](#교차-확인-결과--capacity-gap-이-아니라-기법--teacher-크기-교호작용이었다)).

### 전체 결과 한눈에 (VOC test2007, `val/` best.pt 재평가)

| # | 런 | 축 | mAP50-95 | 비고 |
|:---:|---|---|:---:|---|
| — | baseline n | — | 0.6284 | student 출발점 |
| — | baseline s / m | — | 0.6750 / 0.7077 | teacher 후보 (VOC 파인튜닝) |
| 1 | head`.1` × s-VOC × MSE | 시작점 | 0.6342 | KD 최초 이득 +0.58pt |
| 2 | head`.1` × **s-COCO** × MSE | teacher 출처 | 0.6496 | **Q1 확정** +1.53pt |
| 3 | neck × s-COCO × MSE | 증류 위치 | 0.6408 | 위치 최하위 |
| 4 | head`.0` × s-COCO × MSE | 증류 위치 | 0.6484 | head`.1` 과 구분 불가 |
| 5 | head`.1` × s-COCO × **PKD** | KD 기법 | 0.6533 | +0.37pt |
| 6 | head`.1` × s-COCO × **MGD** | KD 기법 | 0.6501 | ≈MSE (+0.05pt) |
| 7 | head`.1` × s-COCO × FGD | KD 기법 | — | ep2 중단 (off-label) |
| 8 | head`.1` × s-COCO × **AMSE** | 가중 기준(크기) | 0.6429 | **−0.66pt** |
| 9 | head`.1` × s-COCO × **QMSE** | 가중 기준(품질) | **0.6564** | **최고 — +2.80pt vs baseline** |
| 10 | head`.1` × s-COCO × PQMSE | 마스크 구성 | — | ep17 중단 |
| 11 | head`.1` × **m**-COCO × QMSE | teacher 크기 | 0.6517 | −0.47pt |
| 12 | head`.1` × **m**-COCO × PKD | teacher 크기 | 0.6541 | ≈s (+0.08pt) |
| 13 | **neck** × s-VOC × MSE | 위치 × teacher | 0.6372 | **순위 역전** (neck > head`.1`) |

**최종: `head.1` × s-COCO × QMSE = 0.6564, baseline n 대비 +2.80pt** (teacher s 와의 격차 4.66pt 중 60% 회수).
누적 경로는 0.6284 →(KD·위치·teacher 출처) 0.6496 →(기법) 0.6564 이고, teacher 크기(m)는 이득이 없었다.

**두 개의 교호작용이 이 실험의 방법론적 결론이다** — 위치 × teacher 출처(+1.17pt, 순위 역전)와
기법 × teacher 크기(−0.56pt). 한 축씩 고정하는 greedy 탐색으로는 두 경우 모두 잘못 일반화할 뻔했고,
2×2 를 채운 뒤에야 "head 가 낫다"·"큰 teacher 가 나쁘다"가 조건부 결론임이 드러났다.

### KD — teacher 크기 축 (head`.1` × QMSE·PKD 2×2, 100에폭, batch 16)

`val/` 의 `best.pt` 재평가 기준. teacher 만 s→m 으로 바꾼 한 축 비교다.

| teacher | teacher 자체 성능 | mAP50 | mAP50-95 | Precision | Recall | Δ (vs s) |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **s-COCO** | (VOC 파인튜닝 시 0.6750) | **0.8575** | **0.6564** | 0.8230 | **0.7853** | — |
| m-COCO | (VOC 파인튜닝 시 0.7077) | 0.8549 | 0.6517 | 0.8248 | 0.7811 | **−0.47pt** |

**더 강한 teacher 가 더 나쁜 student 를 만들었다 — capacity gap 의 교과서적 사례다.**
m 은 VOC 에서 s 보다 +3.27pt 강한 모델인데(0.7077 vs 0.6750), 증류 결과는 −0.47pt 뒤진다.

- **94/100 에폭에서 s 에 뒤졌고**(우세 6/100) 격차가 ep24 이후 −0.3~−0.9pt 로 안정적이다 —
  한 번의 우연이 아니라 전 구간 일관된 열세다. 클래스 개선도 6/20 에 그친다
  (하락: cow −3.11, motorbike −1.55, boat −1.45).
- 다만 **KD 자체는 작동한다** — baseline n(0.6284) 대비 +2.33pt 이고, MSE×s(0.6489)보다도 근소하게 위다
  (67/100 에폭 우세). "m teacher 가 해롭다"가 아니라 **"m 의 이득이 s 보다 작다"** 가 정확한 표현이다.
- 셋업 문제는 배제했다 — config diff 는 `teacher.model`·`weight` 뿐, yolov8m 에서도 Detect 가
  index 22 이고 `cv2/cv3.*.1` 이 같은 Conv(BN+SiLU) 2번째 층이며, QMSE 마스크 건강도도 s 와
  거의 동일하다(IoU 참여율 34.1% vs 33.2%). weight 도 프로브로 맞춰 KD 비중이 53.6% 로 일치한다.

**teacher 크기 추이 3점이 s 에서 정점을 찍는다: 없음 0.6284 → s 0.6564 → m 0.6517.**
student(n, 3.2M)와 teacher 의 격차가 벌어질수록 feature 를 따라가기 어려워진다는 해석과 맞고,
"teacher 의 task 성능"과 "teacher 의 증류 가치"가 다르다는 [Q1 의 결론](#q1-확정--coco-pretrained-teacher-가-voc-학습본을-153pt-로-이긴다)을
크기 축에서 한 번 더 확인한 셈이다 (Q1 은 출처 축에서 같은 이야기를 했다).

#### 교차 확인 결과 — capacity gap 이 아니라 **기법 × teacher 크기 교호작용**이었다

m×PKD 를 추가해 2×2 를 채웠더니 **두 기법의 방향이 갈렸다.**

| mAP50-95 | s-COCO | m-COCO | Δ (m − s) | m>s 에폭 |
|---|:---:|:---:|:---:|:---:|
| **QMSE** | **0.6564** | 0.6517 | **−0.47pt** | 6/100 (ep50 이후 **0/51**) |
| **PKD** | 0.6533 | 0.6541 | +0.08pt | 36/100 (ep50 이후 16/51) |
| | | | **교호작용 −0.56pt** | |

**PKD 에서는 teacher 크기가 중립이다** — +0.08pt 는 노이즈 기준(0.3pt)의 1/4 이고 에폭별 우세도
36/100 으로 동전 던지기다. 반면 **QMSE 에서는 m 이 일관되게 진다** (ep50 이후 51에폭 연속 열세).
즉 앞 절의 "capacity gap" 해석은 **기법 한 종에서만 성립하는 것이었다** — 교차 확인이 없었으면
"큰 teacher 는 나쁘다"로 잘못 일반화할 뻔했다.

**왜 기법에 따라 갈리는가 — 크기 정보 가설로 설명된다.** PKD 는 채널별 표준화로 **activation
크기를 지우고 패턴만** 증류하고, QMSE 는 공간 가중만 얹은 MSE 라 **크기를 그대로 맞춘다.**
m 은 cls 지점 채널이 192(s 는 128)로 넓고 feature 통계도 달라서, 크기까지 맞추려면 aligner(64→192)의
부담이 커진다 — 그 부담을 지는 QMSE 만 손해를 본다. 프로브 실측이 이 그림과 맞물린다:
**PKD 의 ep1 kd_loss 가 m 에서 오히려 11% 낮다**(0.578 vs 0.648) — `1−r` 기준이라 student 는
m 의 *패턴*을 s 보다 더 잘 따라간다는 뜻이다. 따라가기 쉬운데 크기까지 맞추라면 손해가 나는 것이다.

이는 기법 축의 결론([§6 기법 스윕](#kd--기법-스윕-head1--s-coco-고정-100에폭-batch-16))과 같은 방향이다 —
PKD(크기 제거) 승, AMSE(크기 강조) 패. **"head 분기 증류에서 activation 크기는 노이즈"라는 명제가
teacher 크기 축에서 한 번 더 확인됐다.**

**최종 최고 조합은 여전히 s-COCO × QMSE (0.6564)** 다. m 을 쓰려면 PKD 처럼 크기를 지우는 기법이
짝이 되어야 하고, 그렇게 해도 s×QMSE 를 넘지는 못했다.

**한계** — 각 칸이 seed 1개다. QMSE 의 −0.47pt 는 51에폭 연속 열세라 방향이 단단하지만,
PKD 의 +0.08pt 는 "차이 없음"이지 "m 이 낫다"가 아니다.
...
| AMSE (T=1) | 0.1 | 0.8453 | 0.6429 | 0.8239 | 0.7710 | **−0.66pt** |
| **QMSE** | 0.57 | **0.8575** | **0.6564** | 0.8230 | **0.7853** | **+0.69pt** |
| PQMSE | 0.5 | — | — | — | — | ep17 중단 ([§4](#pqmse--결합-품질-마스크-pgd-식-score_gt클래스05--iou2)) |

**AMSE 의 −0.66pt 는 PKD 의 +0.37pt 와 같은 강도의 증거다** — 100에폭 전부에서 MSE 에 뒤졌고(0/100)
클래스 개선도 3/20 뿐이다. 격차는 ep10 −4.4pt 에서 ep100 −0.62pt 까지 좁혀지지만 한 번도 뒤집히지 않는다.
초반 붕괴는 없었다(ep1 0.4245, 밴드 안) — T=1 완화가 급성 간섭은 없앴지만 순위는 못 바꿨다.

**baseline(0.6284)보다는 +1.45pt 위다.** 즉 AMSE 는 KD 로서 작동하지만 균등 가중 MSE 보다 못하다 —
teacher attention 가중은 head 분기에서 **이득을 주지 못한다.** 단서 둘: ① 실측 초기 KD 비중이 43.1%
(w=0.1 이 T=0.5 프로브 기준이라 T=1 에서는 미달)로 MSE(54.3%)보다 낮아 "강도 부족" 교락이 남고,
② T=1·w=0.1 이 산출값과 다른 사용자 설정이다. 다만 비슷한 비중(45.6%)의 neck 런도 +1.24pt 는
얻었으므로, 비중 차이만으로 −0.66pt 가 설명되지는 않는다.

**FGD 부검 완결.** 전체 기법(FGD, ep2 중단)도 완화한 핵심 성분(AMSE)도 head 에서 MSE 를 못 넘었다.
PKD 와의 대비가 그림을 완성한다 — **PKD 는 채널 표준화로 activation 크기를 제거해서 이겼고(+0.37pt),
AMSE 는 크기가 큰 곳을 강조해서 졌다(−0.66pt).** head 분기 증류에서 activation 크기 정보는
신호가 아니라 노이즈고, 살릴 것은 패턴이라는 방향으로 세 결과가 정렬된다.

**QMSE 가 기법 축 신기록이다 — 0.6564, "가중 기준" 축이 단조로 닫혔다.**

| 가중 기준 | 기법 | mAP50-95 | ΔMSE |
|---|---|:---:|:---:|
| teacher 예측 품질 (box←IoU, cls←score) | **QMSE** | **0.6564** | **+0.69pt** |
| 균등 | MSE | 0.6496 | — |
| activation 크기 | AMSE | 0.6429 | −0.66pt |

- **vs MSE +0.69pt**: 91/100 에폭 우세(ep60 이후 41/41), 클래스 개선 **16/20** 으로 전면적이다
  (상위: cow +2.63, bus +2.48, cat +1.75). AMSE 의 −0.66pt 와 거의 대칭인 크기 —
  같은 골격에서 가중 기준만 바꾼 결과라, "어디를 증류하느냐"가 head 에서 ±0.7pt 를 가르는 축임이 확정됐다.
- **vs PKD +0.31pt**: 노이즈 기준(0.3pt)을 살짝 넘는 수준이지만, 중반에 추월한 뒤 **ep60 이후 34/41
  에폭 우세**로 후반 지배가 일관적이다. 클래스 개선 11/20. 순위를 단단히 하려면 seed 반복이 필요하다.
- best epoch 100 으로 아직 상승 중 — 절대값은 하한이다.
- 해석 단서 (다른 수준들과의 차이): 마스크에 **GT 와 teacher 예측**이 들어간다 — feature 만 보는
  MSE·PKD 와 입력 정보량이 다르다. FGD 실패 사례와 겹쳐 읽으면, GT·예측 정보는 head 증류에
  "그 자체로 이득"이 아니라 **어떻게 쓰느냐**(FGD 의 기하학적 사각형 마스크는 해악, QMSE 의
  soft 품질 마스크는 이득)의 문제다.
