...
loss = mean(w_s · w_c · (s − t)²)        # T = 0.5 (FGD 논문 기본값)
```
...
- **head feature 에서 channel attention 은 거의 one-hot 으로 퇴화한다** (실제 VOC 이미지 실측).
  ...
  off-label 문제의 또 다른 단면. **그래도 T=0.5 를 유지한다** — 이 실험의 정의가 "FGD 가 정의한
  가중의 분리 검증"이고, AMSE 가 무너지면 이 one-hot 집중이 유력한 메커니즘으로 남는다.
  temp 완화(T=1~2) 변형은 그 결과를 본 뒤의 후속이다.
